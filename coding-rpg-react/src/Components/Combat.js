...
function Combat(props) {
    ...
    const [preparedSpell, setPreparedSpell] = useState()

    function RunCombat(hero, allies, enemies, target, combatLog, action, spell) {
        CombatRound(hero, allies, enemies, target, combatLog, action, spell);
        var newHero = hero;
        setHero(newHero)
    ...
    function handleConsumable(hero, allies, enemies, target, combatLog, action, spell, inventory, item) {
        ...
        RunCombat(hero, allies, enemies, target, combatLog, spell, action)
        setAction("Attack")
    }
    ...
    const alliesList = allies.filter(ally => ally.CurrentHP > 0).map((ally, index) => <h4 key={index}>{ally.Name} - HP {ally.CurrentHP}/{ally.MaxHP}, MP {ally.CurrentMP}/{ally.MaxMP}, SP {ally.CurrentSP}/{ally.MaxSP} <button onClick={() => RunCombat(hero, allies, enemies, ally, combatLog, action, preparedSpell)}><h4>Target</h4></button></h4>)
    const enemiesList = enemies.filter(enemy => enemy.CurrentHP > 0).map((enemy, index) => <h4 key={index}>{enemy.Name} - HP: {enemy.CurrentHP}/{enemy.MaxHP}, MP: {enemy.CurrentMP}/{enemy.MaxMP}, SP: {enemy.CurrentSP}/{enemy.MaxSP}  <button onClick={() => RunCombat(hero, allies, enemies, enemy, combatLog, action, preparedSpell)}><h4>Target</h4></button></h4>)
