...
function Party(props) {
    ...
    //     }
    // }
    const partyList = party.map((companion, index) => <h3 key={index}>Name: {companion.Name} <button onClick={() => handleCompanionView(companion, index)}><h3>View Character Sheet</h3></button></h3>)
    if (active === "Party") {
        if (party.length > 0) {
            return (<div>
