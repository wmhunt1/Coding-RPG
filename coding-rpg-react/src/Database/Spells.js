...
export function magicMissile() {
    var magicMissile = { Name: "Magic Missile", ManaCost: 5, Description: "Missile of Magical Forcer that deals 5 Force Damage to One Foe", Amount: 5, DamageType: forceDamage(), SpellEffect(char, allies, enemies, target, combatLog) { AddToCombatLog(combatLog, char.Name + " fires a " + this.Name + " at " + target.Name); ProjectileMagicAttack(char, target, combatLog, this) } }
    return magicMissile;
}
//healing spells
export function basicHeal() {
    var basicHeal = { Name: "Basic Heal", ManaCost: 5, Description: "Heals One Ally for 5HP", Amount: 5, DamageType: "Hone", SpellEffect(char, allies, enemies, target, combatLog) { AddToCombatLog(combatLog, char.Name + " casts " + this.Name + " on " + target.Name + ", healing them for" + this.Amount + " Health"); HealHP(target, this.Amount) } }
    return basicHeal;
}